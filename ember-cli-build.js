...
function transpileTree(tree, env) {
  ...
  if (SHOULD_TRANSPILE || env === 'production') {
    transpiled = toES5(stripForProd(transpiled));
  }
...
module.exports = function() {
  ...
  let dist = new MergeTrees([
    ...
    new Funnel(emberDependencies(), { destDir: 'dependencies' }),
  ]);
...
function emberDependencies(environment) {
  ...
  return new MergeTrees([
    ...
    ...glimmerDependenciesES(environment),
  ]);
}
...
function templateCompilerBundle(emberPackages, env) {
  ...
  return concatBundle(new MergeTrees([templateCompilerFiles, internalLoader()]), {
...
function glimmerDependenciesES(environment) {
